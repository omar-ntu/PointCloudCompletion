...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train PointNet VAE on complete point clouds.")
    ...
    parser.add_argument("--out", type=str, default="checkpoints/point_vae.pt")
    ...
    model = PointVAE(args.latent_dim, args.n_full).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in tqdm(loader, desc=f"epoch {epoch:03d}"):
            ...
            recon, mu, logvar, _ = model(full)
            loss = chamfer_distance(recon, full) + args.beta * kld_loss(mu, logvar)
            opt.zero_grad(); loss.backward(); opt.step()
            losses.append(float(loss.detach().cpu()))
        print(f"epoch={epoch:03d} loss={sum(losses)/len(losses):.6f}")
